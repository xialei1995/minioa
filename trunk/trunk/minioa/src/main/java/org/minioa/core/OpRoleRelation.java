...
public class OpRoleRelation {
	...
	private List<OpRoleRelation> recordsList;

	public List<OpRoleRelation> getRecordsList() {
		if (recordsList == null)
			buildRecordsList();
		return recordsList;
	}
	...
	public void buildRecordsList() {
		try {
			...
			while (it.hasNext()) {
				...
				recordsList.add(new OpRoleRelation(FunctionLib.getInt(obj[0]), FunctionLib.getString(obj[3]), FunctionLib.getString(obj[4]), FunctionLib.getInt(obj[5])));
			}
			it = null;
		} catch (Exception ex) {
			ex.printStackTrace();
