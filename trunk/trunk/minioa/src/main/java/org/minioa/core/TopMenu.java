...
public class TopMenu {
	...
	private List<TopMenu> recordsList;

	public List<TopMenu> getRecordsList() {
		if (recordsList == null)
			buildRecordsList();
		return recordsList;
	}
	...
	public TopMenu(String type, int i, String name) {
		setType(type);
		setID_(i);
		setMenuName(name);
	}

	public TopMenu(int parent, int id, String pName, String name, String url, String target) {
		setParentId(parent);
		setID_(id);
		setParentName(pName);
		setMenuName(name);
	...
	public TopMenu(int id, int cId, String cDate, int mId, String mDate, String uuid, int parent, String name, String url, String target, int seq) {
		setID_(id);
		setCID_(cId);
		setCDATE(cDate);
		setMID_(mId);
	...
	public void buildRecordsList() {
		try {
			...
			while (it.hasNext()) {
				...
				recordsList.add(new TopMenu(id, cId, cDate, mId, mDate, uuid, parent, name, url, target, seq));
				rowcount++;
			}
			it = null;

			mySession.getTempInt().put("TopMenu.rowcount", rowcount);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	}
	...
	public void updateRecordById() {
		try {
			...
			Query query = getSession().getNamedQuery("core.topmenu.updaterecordbyid");
	...
	public void deleteRecordById() {
		try {
			...
			if (!FunctionLib.isNum(id))
				return;
			Query query = getSession().getNamedQuery("core.topmenu.deleterecordbyid");
			query.setParameter("id", id);
			query.executeUpdate();
			query = null;
			String msg = getLang().getProp().get(getMySession().getL()).get("success");
	...
	public void moveRecordById() {
		try {
			...
			if (!FunctionLib.isNum(parentId) || !FunctionLib.isNum(id))
				return;
			getMySession().getTempStr().put("TopMenu.move.id", id);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	}
	...
	public void processSelection(NodeSelectedEvent event) {
		try {
			...
			TopMenu bean = (TopMenu) tree.getRowData();
			ID_ = bean.getID_();
			selectRecordById(String.valueOf(ID_));
			parentId = bean.getParentId();
			menuName = bean.getMenuName();
			parentName = bean.getParentName();
			if (FunctionLib.isNum(getMySession().getTempStr().get("TopMenu.move.id"))) {
				Query query = getSession().getNamedQuery("core.topmenu.moverecordbyid");
				query.setParameter("mId", 0);
				query.setParameter("parentId", ID_);
				query.setParameter("id", getMySession().getTempStr().get("TopMenu.move.id"));
				query.executeUpdate();
				query = null;
				getMySession().getTempStr().put("TopMenu.move.id", "");
				FunctionLib.refresh();
			}
		} catch (Exception ex) {
			ex.printStackTrace();
