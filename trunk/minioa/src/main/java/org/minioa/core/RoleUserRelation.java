...
public class RoleUserRelation {
	...
	private List<RoleUserRelation> recordsList;

	public List<RoleUserRelation> getRecordsList() {
		if (recordsList == null)
			buildRecordsList();
		return recordsList;
	}
	...
	public void buildRecordsList() {
		try {
			...
			while (it.hasNext()) {
				...
				recordsList.add(new RoleUserRelation(FunctionLib.getInt(obj[0]), FunctionLib.getInt(obj[1]), FunctionLib.getDateString(obj[2]), FunctionLib.getInt(obj[3]), FunctionLib.getInt(obj[4]), FunctionLib.getString(obj[5]), FunctionLib.getString(obj[6])));
			}
			it = null;
		} catch (Exception ex) {
			ex.printStackTrace();
