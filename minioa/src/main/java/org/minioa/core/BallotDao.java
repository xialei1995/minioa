...
public class BallotDao {
	...
	public ArrayList<Ballot> buildRecordsList() {
		ArrayList<Ballot> recordsList = new ArrayList<Ballot>();
		try {
			getMySession();
	...
	public Ballot selectRecordById() {
		...
		try {
			...
			if (FunctionLib.isNum(id)) {
				Criteria criteria = getSession().createCriteria(Ballot.class).add(Restrictions.eq("ID_", Integer.valueOf(id))).addOrder(Order.desc("ID_"));
				Iterator<?> it = criteria.list().iterator();
				while (it.hasNext())
					bean = (Ballot) it.next();
			}
