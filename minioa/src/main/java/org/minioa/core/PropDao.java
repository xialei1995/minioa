...
public class PropDao {
	...
	public ArrayList<Prop> buildRecordsList() {
		ArrayList<Prop> recordsList = new ArrayList<Prop>();
		try {
			getMySession();
			...
			recordsList = new ArrayList<Prop>();
	...
	public Prop selectRecordById() {
		...
		try {
			...
			if (FunctionLib.isNum(id)) {
				Criteria criteria = getSession().createCriteria(Prop.class).add(Restrictions.eq("ID_", Integer.valueOf(id))).addOrder(Order.desc("ID_"));
				Iterator<?> it = criteria.list().iterator();
				while (it.hasNext())
					bean = (Prop) it.next();
				...
				mySession.getTempStr().put(bean.getPropName(), "Y");
			}
		} catch (Exception ex) {
			ex.printStackTrace();
		}
